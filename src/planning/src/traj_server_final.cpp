...
void cmdCallback(const ros::TimerEvent &e) {
  ...
  debug_msg.position.x = intergral_targetx_body;  // 使用position字段存储error_targetx
  debug_msg.position.y = intergral_targety_body;  // 使用position字段存储error_targety
  debug_msg.position.z = std::atan2(cmd.position.y - vins_p.y(), cmd.position.x - vins_p.x());
