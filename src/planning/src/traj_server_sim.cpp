/*
 * traj_server_sim — 仿真实验专用副本（源文件由 traj_server.cpp 复制）
 * - 放宽 mode 0→1、1→2 的位置/速度/航向门限
 * - 移除自动 1→0（MPC 回退）与 2→1（降落回退）
 */
  ...
  if ((triger_mode == 0 || triger_mode == -2)) {
    ...
    if (triger_mode == 1)
    {
      if (reflight_complete){
        targetz = std::min(hc14_dog_pos.z() + land_height_limit[1], std::max(hc14_dog_pos.z() + land_height_limit[0], vins_p.z()));
...
void flag_and_hc14_process_callback(const ros::TimerEvent &event) {
    ...
    // 处理triger_mode切换逻辑（sim：禁止自动 1→0、2→1；门限数值与 traj_server 一致）
    if ((triger_mode == 0 || triger_mode == -2)){
      if (hc14_offset_pos_ready && 
          hc14_offset_yaw_ready && 
          vins_p.z() > hc14_dog_pos.z() + land_height_limit[0] &&
          Eigen::Vector2d(vins_p.x() - hc14_dog_pos.x(), vins_p.y() - hc14_dog_pos.y()).norm() < 1.5 && 
          angle_diff < 30.0/180.0 * M_PI)
      {
          ...
          mode_msg.pose.orientation.w = 1;
          mode_pub_.publish(mode_msg);
          std::cout << "[sim] precise_mode: true" << std::endl;
      }
    }
    else if (triger_mode == 1){
      if (hc14_offset_yaw_ready && 
          hc14_offset_pos_ready &&
          std::fabs(hc14_dog_vel.x() - vins_v.x()) < 0.6 && 
          std::fabs(hc14_dog_vel.y() - vins_v.y()) < 0.6 && 
          std::fabs(hc14_dog_pos.x() - vins_p.x()) < 1.0 &&
          std::fabs(hc14_dog_pos.y() - vins_p.y()) < 1.0)
      {
        land_timer ++;
        if (land_timer > 10)
        {
          ...
          mode_pub_.publish(mode_msg);
          std::cout << "[sim] land_mode: true" << std::endl;
          land_timer = 0;
        }
      }else{
...
int main(int argc, char **argv) {
  ...
  ROS_WARN("[Traj server SIM]: ready (no auto 1->0/2->1).");
