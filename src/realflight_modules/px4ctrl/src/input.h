...
public:
  ...
  Eigen::Vector3d p;
  Eigen::Vector3d r_p = {0,0,0};
  Eigen::Vector3d v;
  Eigen::Quaterniond q;
  Eigen::Vector3d w;

  nav_msgs::Odometry msg;
  flow_publisher::FlowDataMsg msg2;
  sensor_msgs::Imu msg3;
  ros::Time rcv_stamp;
  bool recv_new_msg;

  double Odom_State = 1;
  double odom_triger = 0;
  double flow_flag = false;
  Odom_Data_t();
  void feed1(nav_msgs::OdometryConstPtr pMsg);
  void feed2(const boost::shared_ptr<const flow_publisher::FlowDataMsg>& data);
  // void feed3(sensor_msgs::ImuConstPtr pMsg);
};
...
//   px_comm::OpticalFlow msg;
//   ros::Time rcv_stamp;
//   bool recv_new_msg;

//   Flow_Data_t();
//   void feed(px_comm::OpticalFlow pMsg);
// };
