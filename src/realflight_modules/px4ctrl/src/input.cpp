...
void Odom_Data_t::feed1(nav_msgs::OdometryConstPtr pMsg)
{
    ...
    // if (Odom_State == 1)
    // {
    // // r_p(0) = p(0) - msg.pose.pose.position.x;
    // // r_p(1) = p(1) - msg.pose.pose.position.y;
    // r_p(2) = p(2) - msg.pose.pose.position.z;
    // }
...
void Odom_Data_t::feed2(const boost::shared_ptr<const flow_publisher::FlowDataMsg>& pMsg)
{
    if (Odom_State == 2)
        return;
    ros::Time now = ros::Time::now();
...
// #define VEL_IN_BODY
#ifdef VEL_IN_BODY /* Set to 1 if the velocity in odom topic is relative to current body frame, not to world frame.*/
    Eigen::Quaternion<double> wRb_q(1,0,0,0);
    Eigen::Matrix3d wRb = wRb_q.matrix();
    // v = wRb * v;
...
// void Odom_Data_t::feed3(sensor_msgs::ImuConstPtr pMsg)
// {
//     if (Odom_State != 1)
//         return;
...
//     q.x() = -msg3.orientation.y;
//     q.y() = msg3.orientation.x;
//     q.z() = msg3.orientation.w;
//     q.w() = -msg3.orientation.z;
