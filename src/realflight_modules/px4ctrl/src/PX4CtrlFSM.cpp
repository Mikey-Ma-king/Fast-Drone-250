...
void PX4CtrlFSM::process()
{
	...
	case MANUAL_CTRL:
	{
		...
		else if (param.takeoff_land.enable && takeoff_land_data.triggered && takeoff_land_data.takeoff_land_cmd == quadrotor_msgs::TakeoffLand::TAKEOFF) // Try to jump to AUTO_TAKEOFF
		{
			...
			// if (odom_data.flow_flag == true)
			// 	ROS_ERROR("[px4ctrl] flow_data received!");
			// else
			// {
			// 	ROS_ERROR("[px4ctrl] no flow_data!");
			// 	break;
			// }
			state = AUTO_TAKEOFF;
			controller.resetThrustMapping();
	...
	case AUTO_HOVER:
	{
		...
		else
		{
				...
				(takeoff_land.delay_trigger.first && now_time > takeoff_land.delay_trigger.second))
			{
				...
				nh.setParam("/px4ctrl/takeoff_triger", 1);
	...
	case AUTO_TAKEOFF:
	{
		// std::cout<< "odom_data:"<<odom_data.p(2)<<"  "<<takeoff_land.start_pose(2)<< " "<< param.takeoff_land.height<<std::endl;
		if (odom_data.p(2) <= (takeoff_land.start_pose(2) + 0.04) && false)
			takeoff_land.start_pose.head<2>() = odom_data.p.head<2>();
		...
		{
			odom_data.Odom_State = 2;
			cmd_data.r_p = odom_data.r_p;
			state = AUTO_HOVER;
			set_hov_with_odom();
...
void PX4CtrlFSM::land_detector(const State_t state, const Desired_State_t &des, const Odom_Data_t &odom)
{
	...
	constexpr double VELOCITY_THR_C = 5.0;		  // Constraint 2: velocity below VELOCITY_MIN_C m/s.
	constexpr double TIME_KEEP_C = 0.07;			  // Constraint 3: Time(s) the Constraint 1&2 need to keep.
	...
	{
		bool C12_satisfy = odom.v.norm()*0 < VELOCITY_THR_C;
...
Desired_State_t PX4CtrlFSM::get_takeoff_land_des(const double speed)
{
	...
	const double VEL_DECAY_TIME = 2.0; // 2 seconds decay time constant
...
{
	// if ((now_time - cmd_data.rcv_stamp).toSec() < param.msg_timeout.cmd)
	// {
	// 	odom_data.Odom_State = 2;
	// 	cmd_data.r_p = odom_data.r_p;
	// }
