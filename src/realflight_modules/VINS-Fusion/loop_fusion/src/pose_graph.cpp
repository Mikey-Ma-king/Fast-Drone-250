...
void PoseGraph::savePoseGraph()
{
    ...
    for (it = keyframelist.begin(); it != keyframelist.end(); it++)
    {
        ...
        FILE *keypoints_file, *brief_file;
        keypoints_file = fopen(keypoints_path.c_str(), "w");
        brief_file = fopen(brief_path.c_str(), "w");
        for (int i = 0; i < (int)(*it)->keypoints.size(); i++)
        {
            uint8_t tmp_c=0, tmp_n=0;
            uint32_t size = (*it)->brief_descriptors[i].size();
            fwrite(&size, sizeof(size), 1, brief_file);
            for (size_t j = 0; j < (*it)->brief_descriptors[i].size(); j++){
                tmp_c = tmp_c | ((*it)->brief_descriptors[i][j] << tmp_n);
                tmp_n ++;
                if (tmp_n == 8){
                    fputc(tmp_c, brief_file);
                    tmp_n = 0;
                    tmp_c = 0;
                }
            }
...
void PoseGraph::loadPoseGraph()
{
                                    ...
                                    &keypoints_num) != EOF) 
    {
        ...
        */
        ...
        brief_file = fopen(brief_path.c_str(), "r");
        ...
        for (int i = 0; i < keypoints_num; i++)
        {
            ...
            uint32_t size;
            fread(&size, sizeof(size), 1, brief_file);

            uint8_t tmp_c = getc(brief_file), tmp_n=0;
            for (wint_t j = 0; j < size; j++){
                tmp_des.push_back(tmp_c & (1 << tmp_n));
                tmp_n ++;
                if (tmp_n == 8){
                    tmp_c = fgetc(brief_file);
                    tmp_n = 0;
                }
            }
