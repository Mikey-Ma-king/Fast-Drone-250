    ...
    while (true)
    {
        ...
        if (markerIds.size() > 0)
        {
            ...
            for (int i = 0; i < markerIds.size(); ++i)
            {
              ...
              center.x /= 4.0;
              center.y /= 4.0;
              double image_center_x = 1280;
              double image_center_y = 720;
              double lock_im = 160.0;            
              ...
              //画面优化
              if (fabs(center.x - image_center_x) > lock_im){
                if(fabs(im_kp) <= 0.1){
                    if(image_center_x - center.x > 0)
                        im_kp += 0.01;
                    else if(image_center_x - center.x < 0)
                        im_kp -= 0.01;}
              }
              else
                im_kp = 0;
            ...
            //机械狗标定
              if(map_triger>15){
                ...
                if(flow_z > 0.3)
                    gos_vins_p0.z() = T1[2] - (flow_z - 0.47);
            ...
            //多标签位置计算
              ...
              for (size_t j = 0; j < markerIds.size(); ++j) {
                int id = markerIds[j];
                Eigen::Vector3d ab_pos(tvecs[j][1], tvecs[j][0], -tvecs[j][2]);
                Eigen::Vector3d positioni = R1 * ab_pos + T1;
                if (id == 1){
                    positioni.x() = positioni.x() + 0.06*std::sin(fin_deg);
                    positioni.y() = positioni.y() - 0.06*std::sin(fin_deg);
                }
                if (id == 2){
                    positioni.x() = positioni.x() - 0.06*std::sin(fin_deg);
                    positioni.y() = positioni.y() + 0.06*std::sin(fin_deg);
                ...
                markerPositions[id] = positioni;
                averagePosition += positioni;
                positionCount++;
              }
            ...
            //降落位置识别
              ...
              glo_pos = position;
              if (flag1 % 1 == 0) {
                    double siny_cosp_land = 2.0 * (q.w() * q.z() + q.x() * q.y());
                    double cosy_cosp_land = 1.0 - 2.0 * (q.y() * q.y() + q.z() * q.z());
                    double filtered_x = kf_x.filter(position[0]);
                    double filtered_y = kf_y.filter(position[1]);
                    double filtered_z = kf_z.filter(position[2]);
                    Eigen::Vector3d _pos{position[0], position[1], position[2]};
                    double gos_angle = std::atan2(siny_cosp_land, cosy_cosp_land);
                    ...
                    odom_msg.pose.pose.orientation.w = std::atan2(siny_cosp_land, cosy_cosp_land);
