...
# 定义目标函数
def objective(delta_rr):
    """
    目标函数：最小化绝对误差和
    delta_r : 待优化的偏移量 [delta_x, delta_y]
    """
    global delta_r
    delta_rr = np.array(-delta_rr)
    errors = []
    for i in range(len(data_list)):
        # 计算修正后的理论距离
        corrected_distance = np.linalg.norm(data_list[i][0] - data_list[i][1] - delta_rr)
        # 计算与实测距离的绝对误差
        errors.append((corrected_distance - data_list[i][2])**2)
    if (np.linalg.norm(delta_rr - delta_r) > 0.3):
...
class uwb_listerer:
    def __init__(self):
        # 订阅 /vins_fusion/imu_propagate 主题
        self.pose_sub = rospy.Subscriber('AOA_Tag_data', Odometry, self.pose_cb)
        # 用于保存T1和R1
    ...
    def pose_cb(self, msg):
        ...
        if len(data_list) > 20:
            ...
            optimize_count += 1
            if delta_triger == 1 and optimize_count > 4 and np.linalg.norm(data_list[0][0] - vins_p2d) + np.linalg.norm(data_list[0][1] - target_p2d) > 0.25:
                # 优化求解
                result = minimize(objective, delta_r, method='L-BFGS-B', bounds=[(delta_r[0]-0.3, delta_r[0]+0.3), (delta_r[1]-0.3, delta_r[1]+0.3)])
                delta_r = result.x
                ...
                print("delta_r:",delta_r,f"       error:{objective(delta_r):.4f}")
                optimize_count = 0
                
class uwb_a0:
    def __init__(self):
        # 订阅 /vins_fusion/imu_propagate 主题
        self.pose_sub = rospy.Subscriber('/uwb/odometry/a0', Odometry, self.pose_cb0)
        self.pose_sub = rospy.Subscriber('/uwb/odometry/a3', Odometry, self.pose_cb3)
        # 用于保存T1和R1
        # 触发条件的标志
        
    def pose_cb0(self, msg):
        global distancea0
        distancea0 = msg.pose.pose.position.x

    
    def pose_cb3(self, msg):
        global distancea3
        distancea3 = msg.pose.pose.position.x
...
if __name__ == '__main__':
    ...
    unsure_target_pxy = np.array([-10,-10])

    while np.linalg.norm(target_pxy - vins_pxy) > 1 or True:
        if delta_triger == 1 and np.linalg.norm(unsure_target_pxy - vins_pxy) > 2:
            p.x = 2*(unsure_target_pxy[0] - vins_pxy[0])/np.linalg.norm(unsure_target_pxy - vins_pxy) + vins_pxy[0]
            p.y = 2*(unsure_target_pxy[1] - vins_pxy[1])/np.linalg.norm(unsure_target_pxy - vins_pxy) + vins_pxy[1]
            p.z = max(vins_p[2],1.5)
            v_x = 1.2*(unsure_target_pxy[0] - vins_pxy[0])/np.linalg.norm(unsure_target_pxy - vins_pxy)
            v_y = 1.2*(unsure_target_pxy[1] - vins_pxy[1])/np.linalg.norm(unsure_target_pxy - vins_pxy)
            v_z = max(min(0.4 * (1.5 - vins_p[2]),0.5),-0.5)
        elif delta_triger == 1 and np.linalg.norm(unsure_target_pxy - vins_pxy) <= 2:
            unsure_yaw += 0.0015*rotater_flag
            if unsure_yaw > 0.69:
                rotater_flag = -1
            elif unsure_yaw < -0.69:
                rotater_flag = 1
            p.x = unsure_target_pxy[0]
            p.y = unsure_target_pxy[1]
            p.z = max(vins_p[2],1.5)
            v_x = 0.7*(unsure_target_pxy[0] - vins_pxy[0])
            v_y = 0.7*(unsure_target_pxy[1] - vins_pxy[1])
            v_z = max(min(0.4 * (1.5 - vins_p[2]),0.5),-0.5)
        elif delta_triger == 0 and np.linalg.norm(target_pxy - vins_pxy) > 2:
            p.x = 2*(target_pxy[0] - vins_pxy[0])/np.linalg.norm(target_pxy - vins_pxy) + vins_pxy[0]
            p.y = 2*(target_pxy[1] - vins_pxy[1])/np.linalg.norm(target_pxy - vins_pxy) + vins_pxy[1]
            p.z = max(vins_p[2],1.5)
            v_x = 1.2*(target_pxy[0] - vins_pxy[0])/np.linalg.norm(target_pxy - vins_pxy)
            v_y = 1.2*(target_pxy[1] - vins_pxy[1])/np.linalg.norm(target_pxy - vins_pxy)
            v_z = max(min(0.4 * (1.5 - vins_p[2]),0.5),-0.5)
        else:
            p.x = target_pxy[0]
            p.y = target_pxy[1]
            p.z = max(vins_p[2]-0.6,target_p[2])
            v_x = 0.9 * (target_pxy[0] - vins_p[0])
            v_y = 0.9 * (target_pxy[1] - vins_p[1])
            v_z = max(min(0.4 * (target_p[2] - vins_p[2]),0.5),-0.5)
            # if (vins_p[2] - target_p[2] < 0.6)
            #     v_z
            if abs(target_pxy[0] - vins_p[0])<=0.8 and abs(target_pxy[1] - vins_p[1])<=0.8 and abs(target_p[2] - vins_p[2])<=0.5 and read_triger == 0:
        ...
        gos_pxy = np.array([gos_p[0],gos_p[1]])
        # if read_triger == 0:
        # print(target_p,yaw,"read:",read_triger)
        # r_pxy = np.array([gos_pxy[0] - dog_p0xy[0] + vins_p0xy[0],gos_pxy[1] - dog_p0xy[1] + vins_p0xy[1]])
        # target_p = np.array([np.linalg.norm(r_pxy)*math.cos(math.atan2(r_pxy[1],r_pxy[0]) - yaw0),np.linalg.norm(r_pxy)*math.sin(math.atan2(r_pxy[1],r_pxy[0]) - yaw0),vins_p0[2] + 0.0])
        # target_yaw = gos_yaw - yaw0
        r_p = gos_p - dog_p0
        ...
        r_p[1] = rotated_y
        target_p = r_p + dog_vins_p0
        target_p[0] -= delta_r[0]
        target_p[1] -= delta_r[1]
        target_yaw = gos_yaw - dog_yaw0 + dog_vins_yaw0
        target_pxy = np.array([target_p[0] - tracking_d*math.cos(target_yaw),target_p[1] - tracking_d*math.sin(target_yaw)])
        unsure_target_pxy = np.array([target_p[0] - 4*math.cos(target_yaw + unsure_yaw),target_p[1] - 4*math.sin(target_yaw + unsure_yaw)])
        ...
        msg_beacon.pose.pose.orientation.w = target_yaw
        beacon_pub.publish(msg_beacon)

        if objective(delta_r) < 0.4 and delta_triger == 1 and np.linalg.norm(unsure_target_pxy - vins_pxy) < 0.5 and unsure_yaw < -0.3:
            delta_triger = 0
            print("delta_r converege!")
        # print(vins_p,vins_yaw)

        # x_B, y_B = triangulate(distancea0, distancea3, dAB, target_yaw)
        # print(math.atan2(y_B,x_B))
        # x_B = math.atan2(y_B,x_B)
        # print("sssss",find_coordinates(distancea0, distancea3, dAB, target_yaw))
        # (x_A, y_A) = resultion[0][0]
        # (x_B, y_B) = resultion
        # print("distancea0:",distancea0,"distancea3",distancea3)
        # target_uwb_P = np.array([(x_A+x_B)/2 + vins_p[0] , (y_A+y_B)/2 + vins_p[1] , target_p[2] ])
        # print("delta_r:",[target_uwb_P[0] - target_p[0], target_uwb_P[1]-target_p[1]])
        # point_msg = PointStamped()
        # point_msg.header.stamp = rospy.Time.now()
        # point_msg.header.frame_id = "map"
        # point_msg.point.x = 0  # 转换为米
        # point_msg.point.y = 0
        # point_msg.point.z = 0
