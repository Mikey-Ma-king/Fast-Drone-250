...
1. **远距离 / 避障阶段**：MPC 规划平滑轨迹，跟踪狗的世界系位姿，并融合深度图避障。
...
以 **DroneMPC**（Cython 编译的 `MPC_python_complie`）为核心，周期性求解最优轨迹，发布位置/速度/加速度参考，供 `traj_server` 插值跟踪。支持深度图避障与 agent 指令跟踪。
...
| **代价函数** | 跟踪误差 `Q`、控制量 `R`、控制增量 `S`；可选方向误差惩罚 `w_theta`（速度与目标方向叉积） |
| **求解器** | CVXPY + OSQP |
| **目标轨迹预测** | `predict_target_trajectory`：5 点速度历史线性最小二乘拟合，积分得位置；不足 5 点则匀速直线 |
| **轨迹拼接** | `merge_trajectory`：保留旧解未执行段 + 新解尾部，减少重规划抖动 |
| **深度避障** | 深度图 4×4 分格取最近有效深度 → `obstacle_grid`；结合 VINS 四元数反投影为世界系射线方向，传入 `mpc.solve` |
...
- 深度图（默认 `/camera/depth/image_rect_raw`）
...
| **偏航控制** | `cmd.yaw = clamp(angle_diff, ±0.5°) + vins_yaw`；远距离 MPC 可朝向狗 |
