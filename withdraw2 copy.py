...
mpc = DroneMPC(N = 15)
mpc.N = 15
mpc.v_max = np.array([1.5, 1.5, 0.6])
mpc.a_max = np.array([1.0, 1.0, 0.8])
...
def target_update():
    ...
    if (math.cos(math.atan2(target_p1[1] - vins_p[1], target_p1[0] - vins_p[0]) - vins_yaw - AOA_angle) > 0.1) and AOA_init:
        ...
        target_p2[0] = vins_p[0] + AOA_distancexy*math.cos(vins_yaw + AOA_angle)
        target_p2[1] = vins_p[1] + AOA_distancexy*math.sin(vins_yaw + AOA_angle)
        ...
        if (AOA_distance > 1 and abs(AOA_angle) < 80*math.pi/180):
            target_p = 0.95*target_p1 + 0.05*target_p2
            if (np.linalg.norm(target_p2 - target_p1) > 0):
                delta_r += 0.01*(target_p2[:2] - target_p1[:2])/np.linalg.norm(target_p2 - target_p1)
        else:
            target_p = target_p1
...
if __name__ == '__main__':
    ...
    print("开始任务")
    mission_finish = True
    remote_point = np.array([6.0,6.0,1 + vins_p[2] - flow_z])
    ...
    while True:
        ...
        if mpc_count > 20:
            mpc_count = 0
            smooth_target = np.array([target_pxy[0],target_pxy[1],target_p[2]])
            if np.linalg.norm(target_pxy[:2] - vins_p[:2]) > 2.5:
                smooth_target[2] = vins_p[2] - (flow_z - 1.5)
                # smooth_target[2] = 1.5
            elif np.linalg.norm(target_pxy[:2] - vins_p[:2]) > 0.5:
                smooth_target[2] =(vins_p[2] - flow_z + 1.5-target_p[2])*(np.linalg.norm(target_pxy[:2] - vins_p[:2]) - 0.5)/2.0 + target_p[2]
                # smooth_target[2] =(1.5 - target_p[2])*(np.linalg.norm(target_pxy[:2] - vins_p[:2]) - 0.5)/2.0 + target_p[2]
            else:
                smooth_target = np.array([target_pxy[0],target_pxy[1],target_p[2]])
            
            MPC_mission(target_p=smooth_target, target_v=np.array([0,0,0]))
