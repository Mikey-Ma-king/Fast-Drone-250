...
mpc = DroneMPC(N = 15)
mpc.N = 15
mpc.v_max = np.array([1.0, 1.0, 0.8])
mpc.a_max = np.array([0.6, 0.6, 0.6])
...
look_point = np.array([0.0,0.0,0.0])
...
def target_update():
    ...
    look_point = target_p1

    if (AOA_distance > 0):
        target_p = 0.4*target_p1 + 0.6*target_p2
        if (np.linalg.norm(target_p2 - target_p1) > 0):
            delta_r += 0.02*(target_p2[:2] - target_p1[:2])/np.linalg.norm(target_p2 - target_p1)
    else:
        target_p = target_p1
...
if __name__ == '__main__':
    ...
    while mission_finish is False:
        MPC_mission(target_p=np.array([look_point[0],look_point[1],look_point[2] + 1.5]),target_v=np.array([0,0,0]))
