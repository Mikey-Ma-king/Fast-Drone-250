...
from NMPCbackup import DroneNMPC
...
class all_Subscriber:
    ...
    def pose_cb0(self, msg):
        ...
        if(land_triger == 1):
            target_v[0] = msg.twist.twist.linear.x * math.cos(target_yaw)
            target_v[1] = msg.twist.twist.linear.y * math.sin(target_yaw)
            target_v[2] = msg.twist.twist.linear.z
        else:
            target_v[0] = msg.twist.twist.linear.x
            target_v[1] = msg.twist.twist.linear.y
...
def run_while_loop():
    ...
    while not rospy.is_shutdown():
        while triger != 1 or target_received_ != 1:
            time.sleep(0.1)
        target_p_dis[0] = target_p[0] - max(0.2,0 - 0.0*(time.time() - land_time))*math.cos(target_yaw)
        target_p_dis[1] = target_p[1] - max(0.2,0 - 0.0*(time.time() - land_time))*math.sin(target_yaw)
        ...
        if (land_triger == 0 and x_opt is not None or (land_triger == 1 and x_opt is not None and land_init_ == 1)):
            drone_state,accel = traj_get_state(x_opt, u_opt, int((time.time() - MPC_clcyle + shift)/nmpc.dt)*nmpc.dt, nmpc.dt)
        ...
        # target_traj = predict_target_trajectory(target_p_dis, (1 + land_triger/10*0)*target_v, nmpc.N, nmpc.dt)
        start_time = time.time()
        new_x_opt, new_u_opt = nmpc.solve(drone_state, target_p_dis,target_v,target_yaw)
        end_time = time.time()
        ...
        x_opt = merge_trajectory(x_opt, new_x_opt, now=time.time() - MPC_clcyle ,shift=shift, mpc_N=nmpc.N, dt=nmpc.dt)
        # x_opt = new_x_opt
        u_opt = new_u_opt
        MPC_clcyle = time.time() 
        # Traj_splines.update(x_opt,u_opt)
        time.sleep(0.2)
        vis.visualize_traj(x_opt, nmpc.dt, topic="/drone2/planning/traj")
