...
if __name__ == "__main__":
    ...
    def send_offboard_mode():
        """不间断发送setModeServer指令"""
        global keep_sending
        while keep_sending:
            setModeServer(custom_mode='OFFBOARD')
            time.sleep(0.1)  # 设置一个短暂的间隔，防止过于频繁的发送

# 主逻辑部分
    thread = None  # 用于保存线程对象
    ...
    try:
        ...
        while (1):
            if offboard == 1:
                if thread is None or not thread.is_alive():  # 检查线程是否已经在运行
                    keep_sending = True  # 启动发送线程
                    thread = threading.Thread(target=send_offboard_mode)
                    thread.start()
