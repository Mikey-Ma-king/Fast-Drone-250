#!/usr/bin/env python3
"""
实验：target_ekf_odom 频率与 dog_pos 漂移速率对定位误差的影响（热力图）
流程：
0. 需先启动 roscore
1. 清理之前相关参数
2. 设置参数：频率 a~b 取 x 个点，漂移速率 c~d 取 y 个点（漂移为 yaw/offset 同步）
...
6. 将平均误差矩阵及频率、漂移设置保存为 JSON，并绘制热力图（左 frequency，下 drift，右 error，标签加粗）
...
def run_single_trial(freq_hz, drift_rate, duration_sec, recorder, workspace, drift_type='both', wait_pos_ready=False, yaw_drift_rate=None):
    """单次实验：设置频率与漂移速率参数（可选仅 yaw / 仅 pos / 同步）-> 启动 launch 与 fake_target
    -> （可选）等待 dog_pos_processed pos/yaw ready 收敛后再开始 duration 计时 -> 记录 duration_sec -> 关闭 -> 返回平均误差
    yaw_drift_rate: drift_type=both 时单独指定 yaw 漂移速率 (rad/s)，若为 None 则用 drift_rate。"""
    ...
    # drift_type=both 时 yaw 用 yaw_drift_rate（若传入），否则用 drift_rate
...
def main():
    parser = argparse.ArgumentParser(description='Target EKF 频率与 dog_pos 漂移速率对定位误差影响（热力图）')
    parser.add_argument('--freq-min', type=float, default=5.0, help='target_ekf_odom 频率下限 (Hz)')
    parser.add_argument('--freq-max', type=float, default=15.0, help='target_ekf_odom 频率上限 (Hz)')
    parser.add_argument('--freq-num', type=int, default=5, help='频率采样点数')
    parser.add_argument('--drift-min', type=float, default=0.0, help='dog_pos 漂移速率下限（单位 rad/s 或 m/s，取决于 drift_type）')
    parser.add_argument('--drift-max', type=float, default=0.1, help='漂移速率上限')
    parser.add_argument('--drift-num', type=int, default=5, help='漂移速率采样点数')
    parser.add_argument(
        '--drift-type',
        type=str,
        default='both',
        choices=['both', 'yaw', 'pos'],
        help='漂移速率类型：both=同时 yaw_rate 和 pos_rate(x/y)，yaw=仅 yaw_rate，pos=仅 pos_rate(x/y)',
    )
    parser.add_argument('--yaw-drift-min', type=float, default=None,
                        help='drift_type=both 时 yaw 漂移速率下限 (rad/s)；未设则与 drift-min 一致')
    parser.add_argument('--yaw-drift-max', type=float, default=None,
                        help='drift_type=both 时 yaw 漂移速率上限 (rad/s)；未设则与 drift-max 一致')
    parser.add_argument(
        '--wait-pos-ready',
        action='store_true',
        help='是否在计时前等待 dog_pos_processed 的 pos/yaw ready（等收敛后再开始 duration 计时）',
    )
    parser.add_argument('--duration', type=float, default=5.0, help='每次实验记录时长 (s)')
    ...
    rospy.init_node('experiment_target_ekf_freq_drift', anonymous=True)
    ...
    # drift_type=both 时 yaw 漂移范围：未指定则与 drift_min/drift_max 一致
    ...
    if not out_dir:
        out_dir = os.path.join(workspace, f'experiment_freq_drift_{datetime.now().strftime("%Y%m%d_%H%M%S")}')
    ...
    # error_matrix[i][j] = 频率 freq_vals[i], 漂移 drift_vals[j] 时的平均误差（行=freq，列=drift）
    error_matrix = []
    ...
    for fi, freq in enumerate(freq_vals):
        ...
        for dj, drift in enumerate(drift_vals):
            ...
            if shutdown_flag.is_set():
                break
            # drift_type=both 时：pos 用 drift，yaw 从 yaw_drift_min 线性插值到 yaw_drift_max
            if args.drift_type == 'both' and args.drift_num > 1:
                ...
                yaw_drift = None
            rospy.loginfo(
                "Trial %d/%d: freq=%.2f Hz, drift_rate=%.4f, type=%s, wait_pos_ready=%s%s",
                idx, total, freq, drift, args.drift_type, args.wait_pos_ready,
                f", yaw_drift={yaw_drift:.4f}" if yaw_drift is not None else "",
            )
    ...
    data = {
        ...
        'note': (
            'error_matrix[i][j] = mean error at freq_vals[i], drift_vals[j]; '
            f'drift_type={args.drift_type}; pos 用 drift_min~drift_max，'
            f'drift_type=both 时 yaw 用 yaw_drift_min~yaw_drift_max (rad/s).'
        ),
        **({'yaw_drift_min': yaw_drift_min, 'yaw_drift_max': yaw_drift_max} if args.drift_type == 'both' else {}),
    ...
    # 热力图：横轴 drift，纵轴 frequency，颜色 error，带单位，双线性插值平滑过渡
    try:
        import matplotlib
        matplotlib.use('Agg')
        ...
        if M.size > 0 and np.any(np.isfinite(M)):
            ...
            ax.set_xlabel('drift rate (rad/s or m/s, see drift_type)', fontweight='bold')
